...
Maps render via the Google Maps JavaScript SDK in
`src/components/map/GoogleMapBackground.web.tsx` on every platform,
including Capacitor (Android/iOS) where the WebView loads the same JS map.
There is no native Google Maps SDK integration.

API key: `VITE_LOVABLE_CONNECTOR_GOOGLE_MAPS_BROWSER_KEY` in `.env`
(Lovable Google Maps connector). The native Android/iOS Google Maps SDKs
are not used and no manifest meta-data key is required.
